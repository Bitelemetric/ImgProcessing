...
def read_image_folder(images_folder_path = './data/test/'):
...
masked = read_image_folder()
transformed_img = align_all_images(masked)
for i in range(len(transformed_img)):
    plt.imsave('aliniados'+str(i)+'.png',transformed_img[:,:,i],format='png' , cmap='gray')